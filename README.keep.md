...
| `isTouched(path?)`          | `Function`       | Checks if a specific path or the entire form has been touched/focused by the user.     |
| `markDirty(path, dirty?)`   | `Function`       | Programmatically marks a specific field path as dirty manually.                        |
| `markTouched(path, touched?)`| `Function`      | Programmatically marks a specific field path as touched (defaults to true) manually.   |
...
| `schema`                    | `object`         | Getter returning the current active Zod validation schema.                             |
